    ...
    <el-card class="table-container" shadow="hover">
      ...
      <div class="table-area">
        <el-table ref="busTable"
          ...
          <el-table-column label="业态编码" align="center">
            <template slot-scope="scope">{{scope.row.businessType}}</template>
          </el-table-column>
          <el-table-column label="业态名称" align="center">
            <template slot-scope="scope">{{scope.row.businessName}}</template>
          </el-table-column>
          <el-table-column label="业态名称拼音" align="center" width="210">
            <template slot-scope="scope">{{scope.row.namePY}}</template>
          </el-table-column>       
          <el-table-column label="所属企业" align="center" width="210">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="所属业态：" prop="businessType">
              <el-input 
                v-model="dialogData.model.businessType" 
                :maxlength="64" show-word-limit>
              </el-input>
            </el-form-item>
          </el-col>
...
<script>
  import { saveBusinesstypeDetail,getBusinesstypePage } from '../../../api/businesstype'
  import { CommonFlag, UserRoles, flagsToItems, itemsToFlags } from '@/utils/enum'
  ...
  const defaultBusinesstypeModel = {
    id:'',
    businessType: '',
    businessName: '',
    namePY: '',
  ...
  export default {
    ...
    data() {
      return {
        ...
        listLoading: false,
        rowLoading: -1,
        statusOptions: CommonFlag.items(),
        //rolesOptions: UserRoles.items(),
        dialogData: {
          show: false,
          saving: false,
          edit: false,
          model: Object.assign({}, defaultBusinesstypeModel),
          //rolesOptions: UserRoles.items(),
          rules: {
            // account: [
    ...
    created() {
      if(this.$route.query.id) {
       defaultListQuery.id = defaultPositionModel.id = this.listQuery.id = this.$route.query.id
      }
      this.getList()
    },
    filters: {
    ...
    methods: {
      ...
      getList(paged) {
        ...
        getBusinesstypePage(this.listQuery).then(res => {
      ...
      bindCreate() {
        ...
        this.dialogData.model = Object.assign({}, defaultBusinesstypeModel)
        this.dialogData.show = true
        this.$nextTick(() => this.$refs.positionForm.clearValidate())
      },
      resetForm() {
